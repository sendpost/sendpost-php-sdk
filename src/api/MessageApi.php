...
class MessageApi
{
     ...
     * @return \sendpost\model\Message
     */
    public function getMessageById($message_id, string $contentType = self::contentTypes['getMessageById'][0])
    {
        list($response) = $this->getMessageByIdWithHttpInfo($message_id, $contentType);
        return $response;
    }
     ...
     * @throws \sendpost\ApiException on non-2xx response or if the response body is not in the expected format
     * @throws \InvalidArgumentException
     * @return array of \sendpost\model\Message, HTTP status code, HTTP response headers (array of strings)
     */
    public function getMessageByIdWithHttpInfo($message_id, string $contentType = self::contentTypes['getMessageById'][0])
    {
        $request = $this->getMessageByIdRequest($message_id, $contentType);
     ...
     */
    public function getMessageByIdAsync($message_id, string $contentType = self::contentTypes['getMessageById'][0])
    {
        return $this->getMessageByIdAsyncWithHttpInfo($message_id, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function getMessageByIdAsyncWithHttpInfo($message_id, string $contentType = self::contentTypes['getMessageById'][0])
    {
        $returnType = '\sendpost\model\Message';
        $request = $this->getMessageByIdRequest($message_id, $contentType);
     ...
     */
    public function getMessageByIdRequest($message_id, string $contentType = self::contentTypes['getMessageById'][0])
    {

        // verify the required parameter 'message_id' is set
        if ($message_id === null || (is_array($message_id) && count($message_id) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $message_id when calling getMessageById'
            );
        }


        $resourcePath = '/account/message/{message_id}';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
